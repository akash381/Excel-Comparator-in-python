...
def compareExcel():    
    ...
    for s in range(0, noOfSheets1): # Update this to include criteria sheet : from 0 to noOfSheets1
        ...
        # sh2.insert_rows(1)
        sh2.cell(1, column2 + 1).value = 'Mismatch?'

        for r in range(2, row1 + 1):  # Change this from 1 to row1 + 1 for whole scanning
            mismathRow = 0
            ...
            if (mismathRow > 0):
                sh2.cell(r, column2 + 1).value = 'Yes'
            else:
                sh2.cell(r, column2 + 1).value = 'No'
        if(mismatchFound == 0):
